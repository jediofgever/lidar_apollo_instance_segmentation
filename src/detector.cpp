...
namespace lidar_apollo_instance_segmentation
{
  ...
  bool LidarApolloInstanceSegmentation::detectDynamicObjects(
      const sensor_msgs::msg::PointCloud2 &input,
      vox_nav_msgs::msg::ObjectArray &output)
  {
    if (!trt_common_->isInitialized())
    {
      RCLCPP_ERROR_STREAM(node_->get_logger(), "failed to create tensorrt engine file.");
    ...
    for (std::size_t i = 0; i < output.objects.size(); i++)
    {
      sensor_msgs::msg::PointCloud2 transformed_cloud;
      transformCloud(output.objects.at(i).cluster, transformed_cloud, -z_offset_);
      output.objects.at(i).cluster = transformed_cloud;
    }
